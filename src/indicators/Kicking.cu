...
__global__ void kickingKernel(const float *__restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] =
        is_kicking(open[idx - 1], high[idx - 1], low[idx - 1], close[idx - 1],
...
void Kicking::calculate(const float *open, const float *high, const float *low,
  ...
  kickingKernel<<<grid, block>>>(open, high, low, close, output, size);
