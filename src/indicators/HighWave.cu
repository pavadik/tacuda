...
__global__ void highWaveKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] =
            is_high_wave(open[idx], high[idx], low[idx], close[idx]) ? 1.0f : 0.0f;
    }
}
...
void HighWave::calculate(const float* open, const float* high,
    ...
    highWaveKernel<<<grid, block>>>(open, high, low, close, output, size);
