...
__global__ void piercingKernel(const float *__restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] =
        is_piercing(open[idx - 1], high[idx - 1], low[idx - 1], close[idx - 1],
...
void Piercing::calculate(const float *open, const float *high, const float *low,
  ...
  piercingKernel<<<grid, block>>>(open, high, low, close, output, size);
