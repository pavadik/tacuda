...
__global__ void inNeckKernel(const float *__restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] =
        is_in_neck(open[idx - 1], high[idx - 1], low[idx - 1], close[idx - 1],
...
void InNeck::calculate(const float *open, const float *high, const float *low,
  ...
  inNeckKernel<<<grid, block>>>(open, high, low, close, output, size);
