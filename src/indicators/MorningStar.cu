...
__global__ void morningStarKernel(const float *__restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] = is_morning_star(open[idx - 2], high[idx - 2], low[idx - 2],
...
void MorningStar::calculate(const float *open, const float *high,
  ...
  morningStarKernel<<<grid, block>>>(open, high, low, close, output, size);
