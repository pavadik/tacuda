...
__global__ void riseFall3MethodsKernel(const float *__restrict__ open,
  ...
  if (idx > 3 && idx < size) {
    output[idx] = rise_fall_3_methods(
        ...
        open[idx - 1], high[idx - 1], low[idx - 1], close[idx - 1], open[idx],
...
void RiseFall3Methods::calculate(const float *open, const float *high,
  ...
  riseFall3MethodsKernel<<<grid, block>>>(open, high, low, close, output, size);
