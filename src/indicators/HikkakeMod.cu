...
__global__ void hikkakeModKernel(const float* __restrict__ open,
    ...
    if (idx > 2 && idx < size) {
        output[idx] = hikkake_mod(open[idx - 3], high[idx - 3], low[idx - 3],
                                  close[idx - 3], open[idx - 2],
                                  high[idx - 2], low[idx - 2],
                                  close[idx - 2], open[idx - 1],
                                  high[idx - 1], low[idx - 1],
...
void HikkakeMod::calculate(const float* open, const float* high,
    ...
    hikkakeModKernel<<<grid, block>>>(open, high, low, close, output, size);
