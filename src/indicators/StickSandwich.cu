...
__global__ void stickSandwichKernel(const float* __restrict__ open,
    ...
    if (idx > 1 && idx < size) {
        output[idx] = is_stick_sandwich(open[idx - 2], high[idx - 2],
...
void StickSandwich::calculate(const float* open, const float* high,
    ...
    stickSandwichKernel<<<grid, block>>>(open, high, low, close, output, size);
