...
__global__ void longLineKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] = is_long_line(open[idx], high[idx], low[idx], close[idx]) ? 1.0f : 0.0f;
    }
}
...
void LongLine::calculate(const float* open, const float* high,
    ...
    longLineKernel<<<grid, block>>>(open, high, low, close, output, size);
