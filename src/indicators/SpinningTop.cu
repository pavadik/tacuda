...
__global__ void spinningTopKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] =
            is_spinning_top(open[idx], high[idx], low[idx], close[idx]) ? 1.0f : 0.0f;
    }
}
...
void SpinningTop::calculate(const float* open, const float* high,
    ...
    spinningTopKernel<<<grid, block>>>(open, high, low, close, output, size);
