...
__global__ void shortLineKernel(const float *__restrict__ open,
  ...
  if (idx < size) {
    output[idx] =
        is_short_line(open[idx], high[idx], low[idx], close[idx]) ? 1.0f : 0.0f;
  }
}
...
void ShortLine::calculate(const float *open, const float *high,
  ...
  shortLineKernel<<<grid, block>>>(open, high, low, close, output, size);
