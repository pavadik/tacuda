...
__global__ void upsideGap2CrowsKernel(const float* __restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] =
        is_upside_gap_2_crows(open[idx - 2], high[idx - 2], low[idx - 2],
...
void UpsideGap2Crows::calculate(const float* open, const float* high,
  ...
  upsideGap2CrowsKernel<<<grid, block>>>(open, high, low, close, output, size);
