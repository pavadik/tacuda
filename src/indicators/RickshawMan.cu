...
__global__ void rickshawManKernel(const float *__restrict__ open,
  ...
  if (idx < size) {
    output[idx] = is_rickshaw_man(open[idx], high[idx], low[idx], close[idx])
                      ? 1.0f
                      : 0.0f;
  }
}
...
void RickshawMan::calculate(const float *open, const float *high,
  ...
  rickshawManKernel<<<grid, block>>>(open, high, low, close, output, size);
