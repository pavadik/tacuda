...
__global__ void haramiCrossKernel(const float* __restrict__ open,
    ...
    if (idx > 0 && idx < size) {
        output[idx] = harami_cross(open[idx - 1], high[idx - 1], low[idx - 1],
...
void HaramiCross::calculate(const float* open, const float* high,
    ...
    haramiCrossKernel<<<grid, block>>>(open, high, low, close, output, size);
