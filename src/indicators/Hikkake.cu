...
__global__ void hikkakeKernel(const float* __restrict__ open,
    ...
    if (idx > 1 && idx < size) {
        output[idx] = hikkake(open[idx - 2], high[idx - 2], low[idx - 2],
...
void Hikkake::calculate(const float* open, const float* high, const float* low,
    ...
    hikkakeKernel<<<grid, block>>>(open, high, low, close, output, size);
