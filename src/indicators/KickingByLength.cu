...
__global__ void kickingByLengthKernel(const float *__restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] = is_kicking_by_length(open[idx - 1], high[idx - 1],
...
void KickingByLength::calculate(const float *open, const float *high,
  ...
  kickingByLengthKernel<<<grid, block>>>(open, high, low, close, output, size);
