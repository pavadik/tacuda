...
__global__ void separatingLinesKernel(const float *__restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] = separating_lines(open[idx - 1], high[idx - 1], low[idx - 1],
...
void SeparatingLines::calculate(const float *open, const float *high,
  ...
  separatingLinesKernel<<<grid, block>>>(open, high, low, close, output, size);
