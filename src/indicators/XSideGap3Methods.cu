...
__global__ void xSideGap3MethodsKernel(const float* __restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] =
        x_side_gap_3_methods(open[idx - 2], high[idx - 2], low[idx - 2],
...
void XSideGap3Methods::calculate(const float* open, const float* high,
  ...
  xSideGap3MethodsKernel<<<grid, block>>>(open, high, low, close, output, size);
