...
__global__ void longLeggedDojiKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] =
            is_long_legged_doji(open[idx], high[idx], low[idx], close[idx]) ? 1.0f : 0.0f;
    }
}
...
void LongLeggedDoji::calculate(const float* open, const float* high,
    ...
    longLeggedDojiKernel<<<grid, block>>>(open, high, low, close, output, size);
