...
__global__ void tasukiGapKernel(const float* __restrict__ open,
    ...
    if (idx > 1 && idx < size) {
        output[idx] = tasuki_gap(open[idx - 2], high[idx - 2],
...
void TasukiGap::calculate(const float* open, const float* high,
    ...
    tasukiGapKernel<<<grid, block>>>(open, high, low, close, output, size);
