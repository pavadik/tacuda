...
__global__ void tristarKernel(const float* __restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] = tristar(open[idx - 2], high[idx - 2], low[idx - 2],
...
void Tristar::calculate(const float* open, const float* high,
  ...
  tristarKernel<<<grid, block>>>(open, high, low, close, output, size);
