...
__global__ void stalledPatternKernel(const float* __restrict__ open,
    ...
    if (idx > 1 && idx < size) {
        output[idx] = is_stalled_pattern(open[idx - 2], high[idx - 2],
                                         low[idx - 2], close[idx - 2],
                                         open[idx - 1], high[idx - 1],
                                         low[idx - 1], close[idx - 1],
                                         open[idx], high[idx], low[idx],
                                         close[idx]) ? 1.0f : 0.0f;
    }
}
...
void StalledPattern::calculate(const float* open, const float* high,
    ...
    stalledPatternKernel<<<grid, block>>>(open, high, low, close, output, size);
