...
__global__ void shootingStarKernel(const float *__restrict__ open,
  ...
  if (idx < size) {
    output[idx] = is_shooting_star(open[idx], high[idx], low[idx], close[idx])
                      ? 1.0f
                      : 0.0f;
  }
}
...
void ShootingStar::calculate(const float *open, const float *high,
  ...
  shootingStarKernel<<<grid, block>>>(open, high, low, close, output, size);
