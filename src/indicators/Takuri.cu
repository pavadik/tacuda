...
__global__ void takuriKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] = is_takuri(open[idx], high[idx], low[idx], close[idx]) ?
                          1.0f : 0.0f;
    }
}
...
void Takuri::calculate(const float* open, const float* high, const float* low,
    ...
    takuriKernel<<<grid, block>>>(open, high, low, close, output, size);
