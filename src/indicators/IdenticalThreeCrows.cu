...
__global__ void identicalThreeCrowsKernel(const float *__restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] =
        is_identical_three_crows(open[idx - 2], high[idx - 2], low[idx - 2],
...
void IdenticalThreeCrows::calculate(const float *open, const float *high,
  ...
  identicalThreeCrowsKernel<<<grid, block>>>(open, high, low, close, output,
                                             size);
