...
__global__ void thrustingKernel(const float* __restrict__ open,
  ...
  if (idx > 0 && idx < size) {
    output[idx] = is_thrusting(open[idx - 1], high[idx - 1], low[idx - 1],
...
void Thrusting::calculate(const float* open, const float* high,
  ...
  thrustingKernel<<<grid, block>>>(open, high, low, close, output, size);
