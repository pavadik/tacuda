...
__global__ void gravestoneDojiKernel(const float* __restrict__ open,
    ...
    if (idx < size) {
        output[idx] = is_gravestone_doji(open[idx], high[idx], low[idx], close[idx])
                          ? 1.0f
                          : 0.0f;
    }
}
...
void GravestoneDoji::calculate(const float* open, const float* high,
    ...
    gravestoneDojiKernel<<<grid, block>>>(open, high, low, close, output, size);
