...
__global__ void matchingLowKernel(const float* __restrict__ open,
    ...
    if (idx > 0 && idx < size) {
        output[idx] = is_matching_low(open[idx - 1], high[idx - 1], low[idx - 1], close[idx - 1],
...
void MatchingLow::calculate(const float* open, const float* high,
    ...
    matchingLowKernel<<<grid, block>>>(open, high, low, close, output, size);
