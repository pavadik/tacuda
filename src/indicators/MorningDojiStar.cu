...
__global__ void morningDojiStarKernel(const float *__restrict__ open,
  ...
  if (idx > 1 && idx < size) {
    output[idx] =
        is_morning_doji_star(open[idx - 2], high[idx - 2], low[idx - 2],
...
void MorningDojiStar::calculate(const float *open, const float *high,
  ...
  morningDojiStarKernel<<<grid, block>>>(open, high, low, close, output, size);
