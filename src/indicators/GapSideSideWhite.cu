...
__global__ void gapSideSideWhiteKernel(const float* __restrict__ open,
    ...
    if (idx > 1 && idx < size) {
        output[idx] = is_gap_side_side_white(
...
void GapSideSideWhite::calculate(const float* open, const float* high,
    ...
    gapSideSideWhiteKernel<<<grid, block>>>(open, high, low, close, output, size);
