...
TEST(Tacuda, MINUS_DI) {
  ...
  int p = 3;
  ctStatus_t rc =
      ct_minus_di(high.data(), low.data(), close.data(), out.data(), N, p);
  ASSERT_EQ(rc, CT_STATUS_SUCCESS) << "ct_minus_di failed";
  auto ref = minus_di_ref(high, low, close, p);
